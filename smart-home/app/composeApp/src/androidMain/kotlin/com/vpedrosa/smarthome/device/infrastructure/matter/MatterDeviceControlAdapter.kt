package com.vpedrosa.smarthome.device.infrastructure.matter
...
class MatterDeviceControlAdapter(
    ...
    override suspend fun setColor(deviceId: DeviceId, color: Color) {
        executeWithRetry(deviceId) { pointer ->
            val cluster = ChipClusters.ColorControlCluster(pointer, ENDPOINT_ID)
            // RGB (0–255) → HSV, then map to Matter range: hue 0–254, saturation 0–254
            val (matterHue, matterSaturation) = rgbToMatterHueSaturation(color)
            suspendClusterCommand { cb ->
                cluster.moveToHueAndSaturation(cb, matterHue, matterSaturation, 0, 0, 0)
            }
            Log.d(TAG, "${deviceId.value}: Color -> rgb(${color.red},${color.green},${color.blue}) => hue=$matterHue sat=$matterSaturation")
        }
    }
    ...
    /** Returns (matterHue 0–254, matterSaturation 0–254) from an RGB color. */
    private fun rgbToMatterHueSaturation(color: Color): Pair<Int, Int> {
        ...
        val matterHue = (hue360 / 360f * 254f).toInt().coerceIn(0, 254)
        val matterSaturation = (saturation * 254f).toInt().coerceIn(0, 254)
        return matterHue to matterSaturation
    }
