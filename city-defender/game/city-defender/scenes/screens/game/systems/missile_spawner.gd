...
var _difficulty_manager: DifficultyManager
...
func _process(delta: float) -> void:
	if _difficulty_manager == null:
		return
	_spawn_timer += delta
	if _spawn_timer >= _difficulty_manager.spawn_interval:
		_spawn_timer = 0.0
		if _difficulty_manager.is_wave_time():
			_spawn_wave()
		else:
			_spawn_missile()
