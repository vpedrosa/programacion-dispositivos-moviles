...
@export var purchased_upgrades: Array[StringName] = []
...
func to_dict() -> Dictionary:
	return {
		...
		"purchased_upgrades": purchased_upgrades.map(func(u: StringName) -> String: return String(u)),
...
static func from_dict(data: Dictionary) -> PlayerState:
	...
	var upgrades: Array = data.get("purchased_upgrades", [])
	state.purchased_upgrades.assign(upgrades.map(func(u: Variant) -> StringName: return StringName(u)))
